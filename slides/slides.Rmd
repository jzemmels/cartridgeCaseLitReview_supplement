...
I will discuss A Cartridge Case Comparison Pipeline.

If you have any questions or comments throughout the presentation, feel free to stop me.
We do not need to wait until the end of the presentation.
...
???
Here's the standard CSAFE acknowledgement
...
???
A bit about me:
I am originally from the Twin Cities in Minnesota.
I completed my undergraduate degree in Math and Math Education at Winona State University
I started at Iowa State University in 2018 and received my Master's in 2020.
My creative component was related to cartridge case analysis.
After graduation, I would love to teach in the Denver area.
...
???
Outline:
I will begin with some preliminary concepts related to how firearms and toolmarks identification is performed now.
We will then get into the structure of the dissertation.
Each chapter of the dissertation will be a distinct journal article.
Then we'll have some final conclusions.
...
invisible(knitr::plot_crop("images/cartridgeCasePipelineDiagram_examination.png"))
knitr::include_graphics("images/cartridgeCasePipelineDiagram_examination.png")
```

???
First: some background on firearms and toolmarks identification
I will continually reference this diagram that you can see here.
You can think of this diagram as representing an examination "pipeline" where a gun is fired, evidence such as a cartridge case is collected from this firing, then the forensic examiner performs their "examination," and finally arrives to some decision.
I will provide more details in a few slides.
...
???
When a gun is fired, at the same time as the bullet travels down the barrel, the rest of the metal casing, called a cartridge case, moves backwards and smacks against the back wall of the barrel, called the breech face of the barrel.
Any markings on this breech face are impressed into the surface of the cartridge case, leaving the so-called breech face impressions.
Firearm examiners compare these markings during an examination to determine whether two cartridge cases were fired from the same gun.
...
???
Historically, firearm and toolmarks examiners use these comparison microscopes to compare two pieces of evidence.
Two pieces of evidence are placed on the two stages.
The view of these two compound microscopes are combined into a single view at the top of the diagram.
An example of two bullets under the comparison microscope view is seen on the right.
A examiner analyzes the markings on the bullets to determine similarity. 
An examiner will independently rotate and translate the stages to find the optimal matching position of the evidence.
...
???
These firearm and toolmark examinations can be thought of as an evidence to decision "pipeline" where evidence is collected from a crime scene or suspect's gun, they are examined by the F & T examiner who ultimately makes a decision about whether the evidence originated from the same gun.
For our purposes, it's useful to think of the examination as three distinct steps: preprocessing, comparison, and scoring.
Preprocessing might involve adjusting the lighting conditions of the comparison stage.
...
One problem here is that the preprocess, comparison, and scoring stages are done implicitly by the examiner and is difficult to reproduce exactly.
...
???
So, why should we change Firearm and Toolmark Examinations?
I have pulled some quotes from two documents that we all are very familiar with: the 2009 NAS report and the 2016 PCAST report.
These two reports essentially say that the "scientific" foundation upon which F & T examination is placed is shaky; mainly because there aren't well-defined procedures.
Both of these reports recommend that automatic image-analysis algorithms should be developed to supplement the F & T examination with an objective measure of similarity.
...
???
In-general, algorithms can also be thought of as evidence-to-decision (or at least to score) pipelines.
Many algorithms exist to compare F & T evidence (and I've cited only a few here).
While not necessarily the original authors' intentions, each of these algorithms can, to some degree, be broken down into a set of modular "pieces."
For example, I took this diagram from a paper written by a previous CSAFE student, Xiao Hui Tai.
She and Professor Eddy created an algorithm to compare images of cartridge cases that included various image processing techniques.
Conceptually, this algorithm is explained a set of modular steps.
We will come back to this idea of modularization later.
We will focus primarily on the Congruent Matching Cells method originally introduced in 2013 by John Song of NIST.
...
???
Before we discuss algorithms in greater detail, I wanted to discuss the format of the data we will be using.
The cartridge cases we used are from the "Ames I" study ran by Baldwin et al.
These cartridge cases were scanned using the TopMatch-3D scanner from Cadre Forensics.
You can see the scanner on the left.
These scans consist of a a surface matrix, an example of which you can see on the right, and metadata about the scan.
Together, these are stored in an x3p (XML 3D Surface Profile) file.
I will commonly use "scan" and "x3p" interchangeably in this presentation.
...
???
Are there any questions about the introduction before we dive into the first paper?

We will now discuss the first paper entitled "A Study in Reproducibility: The Congruent Matching Cells Algorithm and cmcR package"
This paper has been submitted to the R Journal and we are currently working on addressing reviewers' comments.
...
???
Our goal for this paper was to implement the Congruent Matching Cells algorithm, using these three papers as our primary reference.
In particular, we have structured 
...
???
We chop one of the scans into a grid of "cells"

Our goal will be to find where each aligns best in the other scan.
This includes finding a rotation and a spatial alignment, a translation, for each cell.
The way we find the best alignment is using the cross-correlation function, which measures the similarity between a cell and the other scan.
...
???
This is what is shown on the bottom of this slide.
Each cell has found a location that maximizes the cross-correlation function with the other scan.
However, we're only looking at the locations for a single rotation of the target scan.
We consider a grid of rotations, say -30 to 30 degrees, and try to find the optimal alignment across all of these rotations.
...
???
This is visualized on this slide.
Each column represents a single rotation and each point in this scatterplot represents where a single cell aligns in the other scan.
You'll notice that when we rotate by -3 degrees, the scatterplot seems to be farely random.
However, as we get closer to 3 degrees, you'll notice that some of the points contract towards a particular translation value.
The points them spread out as we move towards 9 degrees.

We can think about each cell "voting" for where it aligns best in the target scan for each rotation.
If a pair of scans are truly matching, then the cells should "agree" on a particular on a particular translation at the true rotation.
In this case, the cells seem to be agreeing on a translation of 10 units left and 10 units up at a rotation of 3 degrees.
...
- Paper \#3 to JDSSV, Submit by 4/1/2023
...
- Dissertation defense by 4/21/23 (last day for Spring 2023 semester)
---
class: primary-blue
# References
...
### cartridgeInvestigatR: Introduction

```{r}
knitr::include_graphics("images/cartridgeInvestigatRIntro.png")
```

---

### cartridgeInvestigatR: Import

```{r}
knitr::include_graphics("images/cartridgeInvestigatR_import.png")
```

---

### cartridgeInvestigatR: 3D Visualization

```{r}
knitr::include_graphics("images/cartridgeInvestigatR_compareX3Ps.png")
```

---

### cartridgeInvestigatR: Preprocess

```{r}
knitr::include_graphics("images/cartridgeInvestigatR_preprocess.png")
```

---

### cartridgeInvestigatR: Comparison Parameters

```{r}
knitr::include_graphics("images/cartridgeInvestigatR_comparisonParameters.png")
```

---

### cartridgeInvestigatR: Comparison Results

```{r}
knitr::include_graphics("images/cartridgeInvestigatR_comparisonResults.png")
```

---

### cartridgeInvestigatR: Custom Cell

```{r}
knitr::include_graphics("images/cartridgeInvestigatR_customCell.png")
```

---

### cartridgeInvestigatR: Scoring

```{r}
knitr::include_graphics("images/cartridgeInvestigatR_scoring.png")
```